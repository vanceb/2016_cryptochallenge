class Key:
    ...
    def set(self, cipherchar, plainchar, reason, conflict_exception=True):
        ...
        if self.__history_pointer > len(self.__history):
            self.__history = self.__history[:self.__history_pointer]
            self.__key_history = self.__key_history[:self.__history_pointer]
            self.__decode, self.__possibles = self.__key_history[-1]
